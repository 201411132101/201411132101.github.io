...
发现在 2000 年, Wassell \cite{w00} 给出了完整的回答.
然而他的证明看不懂, 故我用基础的数分方法证了一遍.
...
\begin{equation} \label{3}
    x = b^x.
\end{equation}
注意到当且仅当 $ b \in (1, e^{1/e}] $ 时, \eqref{3} 有解, 
...
\begin{equation} \label{6}
    x = b^{b^x} 
\end{equation}
的解. 注意到 \eqref{6} 的解必属于 $ (0, 1) $, 故 \eqref{6} 的解与
...
Case 1) $ b \in [(1/e)^e, 1] $. 此时, 画图知 \eqref{7} 有唯一解, 故
$ \{a_k\}_{k \in \mathbb{N}} $ 收敛到 \eqref{6} 的唯一解,
即 $ x = b^x $ 的唯一解. 
(这是因为 $ x = b^x $ 的解一定是 \eqref{6} 的解, 而这两方程都只有一个解, 故解必相同.)

Case 2) $ b \in (0, (1/e)^e) $. 此时, 由画图及 
...
感谢 xs 的提醒, 从上述证明过程中可提炼出如下引理; 证明略.

\begin{lemma} [谢惠民等, 数学分析习题课讲义(上册), 命题 2.6.2]
    设函数 $ f $ 在 $ [a, b] $ 上单调,
    $ \{x_k\}_{k \in \mathbb{N}} \subset [a, b] $ 且 $ x_{k + 1} = f(x_k) $. 则
    \begin{enumerate}[{\rm(i)}]
        \item 若 $ f $ 单增, 则 $ \{x_k\}_{k \in \mathbb{N}} $ 单调;
        \item 若 $ f $ 单减, 则 $ \{x_{2k}\}_{k \in \mathbb{N}} $
            和 $ \{x_{2k-1}\}_{k \in \mathbb{N}} $ 单调且具有相反的单调性.
    \end{enumerate} 
\end{lemma}
